...
/* ============================================================
   Hero — population (compact) + economy (live GDP) + map
   ============================================================ */
function Hero({ now, epoch, onCity }) {
  ...
  // Live world GDP — $3M/sec nominal growth on top of $110.5T base, anchored to epoch.
  const elapsed = (now - epoch) / 1000;
  const gdpPerSec = 3_000_000;
  const gdpTodayPerSec = gdpPerSec; // displayed as "added today"
  const dayStart = new Date(now); dayStart.setUTCHours(0, 0, 0, 0);
  const secsToday = (now - dayStart.getTime()) / 1000;
  const gdpAddedToday = Math.floor(secsToday * gdpTodayPerSec);
  const gdpNominal = 110_500_000_000_000 + Math.floor(elapsed * gdpPerSec);
  const tradePerSec = 75_000;
  const tradeToday = Math.floor(secsToday * tradePerSec);

  return (
    <div className="hero">
      <div className="hero-feature compact">
        ...
        <div className="sub-counter">
          Net today <span style={{ color: 'var(--success)' }}>+{FMT.int(net)}</span>
        </div>
      ...
      <div className="hero-economy">
        <div className="eye"><span className="live-pip"></span>WORLD GDP · LIVE</div>
        <div className="big-counter">${FMT.int(gdpNominal)}</div>
        <div className="sub-counter">
          Added today <span style={{ color: 'var(--success)' }}>+${FMT.int(gdpAddedToday)}</span>
        </div>
        <div className="splits">
          <div>
            <div className="s-label">GDP growth</div>
            <div className="s-val">$3M / sec</div>
            <div className="s-delta">↑ nominal · IMF</div>
          </div>
          <div>
            <div className="s-label">Trade today</div>
...
/* Placeholder card for countries we haven't detailed yet. */
function CountryPlaceholderCard({ c }) {
  return (
    <div className="country-card placeholder" title={`${c.name} · detail coming soon`}>
      <div className="cc-head">
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span className="flag">{c.flag}</span>
          <div>
            <div className="cc-name">{c.name}</div>
            <div className="cc-region">{c.region} · {c.code}</div>
          </div>
        </div>
      </div>
      <div className="cc-soon">
        <span className="cc-soon-pip"></span>
...
function CountryGrid({ onOpen, filter, allCountries: useAll = true }) {
  const detailedByName = React.useMemo(() => {
    const m = new Map();
    D.countries.forEach(c => m.set(c.name.toLowerCase(), c));
    // also map common aliases
    m.set('united states', D.countries.find(c => c.name === 'United States' || c.id === 'us'));
    return m;
  }, []);
  const source = useAll ? D.allCountries : D.countries.map(c => ({ name: c.name, flag: c.flag, code: c.code, region: c.region }));
  const list = filter ? source.filter(filter) : source;
  return (
    <div className="country-grid">
      {list.map((entry) => {
        const detail = detailedByName.get(entry.name.toLowerCase());
        if (detail) return <CountryCard key={entry.code || entry.name} c={detail} onOpen={onOpen} />;
        return <CountryPlaceholderCard key={entry.code || entry.name} c={entry} />;
      })}
    </div>
  );
}
...
Object.assign(window, {
  ...
  CountryPlaceholderCard, CountryGrid, CountryModal,
});
