...
function Sidebar({ route, onRoute, now, epoch }) {
  const items = [
    { id: 'home',         label: 'Home',            Icon: I.Activity },
    { id: 'countries',    label: 'Countries',       Icon: I.MapPin },
...
function App() {
  ...
  let page = null;
  const featured = {
    feed: tweak.featFeed, top: tweak.featTop, story: tweak.featStory,
    videos: tweak.featVideos, destination: tweak.featDestination,
    search: tweak.featSearch, gear: tweak.featGear,
  };
  if (route === 'home')         page = <HomePage now={now} epoch={epoch} onRoute={onRoute} onCountry={onCountry} onCity={onCity} featured={featured} />;
  ...
  else if (route === 'countries')    page = <DestinationsPage onCountry={onCountry} />;
    ...
    page = c ? <CountryPage country={c} onBack={() => onRoute('countries')} onCountry={onCountry} /> : <div>Country not found.</div>;
  }
  ...
  return (
    <div className="shell" data-accent={tweak.accent}>
      ...
      <TweaksPanel title="Tweaks">
        ...
        <TweakSection label="Home · featured sections">
          <TweakToggle label="Right-now feed"
            value={tweak.featFeed}
            onChange={(v) => setTweak('featFeed', v)} />
          <TweakToggle label="Top of the world"
            value={tweak.featTop}
            onChange={(v) => setTweak('featTop', v)} />
          <TweakToggle label="Featured story"
            value={tweak.featStory}
            onChange={(v) => setTweak('featStory', v)} />
          <TweakToggle label="Featured videos"
            value={tweak.featVideos}
            onChange={(v) => setTweak('featVideos', v)} />
          <TweakToggle label="Featured destination"
            value={tweak.featDestination}
            onChange={(v) => setTweak('featDestination', v)} />
          <TweakToggle label="What the world is searching"
            value={tweak.featSearch}
            onChange={(v) => setTweak('featSearch', v)} />
          <TweakToggle label="Travel gear"
            value={tweak.featGear}
            onChange={(v) => setTweak('featGear', v)} />
        </TweakSection>
      </TweaksPanel>
    </div>
  );
