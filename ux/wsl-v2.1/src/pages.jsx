...
/* ============================================================
   Home / Dashboard
   ============================================================ */
function HomePage({ now, epoch, onRoute, onCountry, onCity, featured: feat = {} }) {
  const tickerIds = ['flights', 'co2', 'tourism', 'hotels'];
  const featured = D.countries.find(c => c.featured);
  // Featured-section visibility. Default everything on; tweaks can hide individual blocks.
  const F = {
    feed: true, top: true, story: true, videos: true, destination: true, search: true, gear: true,
    ...feat,
  };
  return (
    <div>
      ...
      {F.story && <div className="section">
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            ...
            <button className="btn btn-primary btn-sm" style={{ marginTop: 18, alignSelf: 'flex-start' }} onClick={() => onRoute('scroller')}>
              Read the story <I.ArrowRight size={12} />
            </button>
          </div>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', padding: 16, alignSelf: 'start' }}>
      ...
      {F.videos && <div className="section">
      ...
      {F.destination && <div className="section">
      ...
      {F.search && <div className="section">
    ...
    <div>
      <div className="country-page-back">
        <button className="btn btn-ghost btn-sm" onClick={onBack}>
          <I.ArrowLeft size={13} /> All countries
        </button>
        <div className="crumb mono">COUNTRIES · {c.code}</div>
      </div>
      ...
      <div className="section">
        <SectionHead title="More countries" sub="Same region, similar growth" right={
          <button className="btn btn-ghost btn-sm" onClick={onBack}>All countries <I.ChevronRight size={12} /></button>
        }/>
        <CountryGrid onOpen={onCountry} allCountries={false} filter={(cc) => cc.id !== c.id && (cc.region === c.region || cc.accent === c.accent)} />
      </div>
...
function DestinationsPage({ onCountry }) {
  ...
  const regions = ['all', ...Array.from(new Set(D.allCountries.map(c => c.region)))];
  const total = D.allCountries.length;
  const detailed = D.countries.length;
  const filter = (c) => {
  ...
  return (
    <div>
      <PageHead
        crumb="COUNTRIES"
        title={`${total} countries. One console.`}
        sub={`Drill into any country to see population, GDP, visitors, top stays and curated gear. ${detailed} detail pages live so far — the rest are placeholders being filled in.`}
        right={
